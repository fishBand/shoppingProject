  ...
  methods: {
    sendSmsMsg() {
      ...
      if (vm.user.userName.length == 0) {
        ...
        //验证手机号是否已注册
        vm.$post(vm.API.API_URL_CUSTOMER, { userName: vm.user.userName }).then(
          res => {
            if (res.errorCode == -3 && !res.data) {
              //发送短信获取验证码
              vm.$post(vm.API.SEND_SMS, { userName: vm.user.userName }).then(
                res => {
    ...
    registerOpt() {
      ...
      if (vm.user.userName.length == 0) {
        ...
        vm.tips.rpwdTip = null;
        //获取默认账户值
        vm.$get(vm.API.API_URL_EWALLET_DEFAULT, {
          userName: vm.user.userName
        }).then(res => {
          if (res.errorCode == 0 && res.data) {
            vm.user.amount = res.data;
            //注册用户信息
            vm.$post(vm.API.REGISTER, {
              userName: vm.user.userName,
              pwd: vm.user.rpwd,
