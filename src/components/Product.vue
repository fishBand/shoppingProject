            ...
            <div class="col-lg-9 col-md-9 col-sm-9">
              <div class="row">
				...
				<div class="shop-option-over">
						...
						@click="addWish(productDetail.id)"
						data-toggle="tooltip"
						title="添加收藏"
						>
                  ...
                  <div class="clearfix margin-bottom-30">
                    ...
                    <strong>商品简介:{{productDetail.explain}}</strong>
                  </div>
                    ...
                    <div class="text-center" id="">
                      ...
                      <div v-for="option in optionList" :key="option">
                        <div class='optionGroup' option>
                          {{option.name}}:
                          <span v-for="optionValues in option.optionValuesList" :key="optionValues" v-bind:optionId="option.id"
                  ...
                  <div class="clearfix form-inline nomargin">
					  <strong>在手机端查看详情</strong><br>
					  <vue-qr :text="mobileUrl"></vue-qr>
				  </div>
                  <!-- /FORM -->
                  <hr/>
                  <div class='row'>
              ...
              <div class="tab-content padding-top-20">
                ...
                <div role="tabpanel" class="tab-pane fade" v-bind:class="{in:!tabSelected,active:!tabSelected}">
                  ...
                  <template v-for="reviews in reviewsList" >
                    <div class="block margin-bottom-60" :key="reviews">
              ...
              <div class="margin-bottom-60">
                ...
                <div class="">
                  <div v-for="item in hotList" :key="item">
                    <img @click="changePro(item.id)" :src="API.BASE_SERVER_URL+item.defaultImg" width="45" height="45"><br>
					<a @click="changePro(item.id)">{{item.name}}</a><br>
					<span style="text-decoration: line-through;color: gray;" >￥{{item.price/100}}</span>
...
<script>
  ...
  export default {
    ...
    created: function () {
      var params = localStorage.getItem('proid')
      if (params) {
        this.productDetailList (params)
        this.getSkuList (params)
        this.loadreviewsList (params)
        this.reviews.productId = params
        this.params.productId = params
		this.getHotList()
		this.addFootPrint()
      }
	  this.mobileUrl = 'http://localhost:8000/mobile/samsara/productdetail/'+localStorage.getItem('proid')
	  //历史数据 需要协商
    },
    methods: {
		addFootPrint(){
			var vm = this
			vm.$post(vm.API.API_URL_ADD_FOOTPRINT,{
		...
		getHotList(){
			...
			vm.$get(vm.API.API_URL_CATALOG_PRODUCTS_HOT+'/'+proid+'/hot/1/pagesize/3').then(function(res){
				if(res.errorCode == 0){
					vm.hotList = res.data
				}
			})    
      ...
      productDetailList (productId) {
        var vm = this
        vm.$get (vm.API.API_URL_CATALOG_PRODUCT_DETAILS + '/' + productId).then(res =>{
            if (res.errorCode == 0) {
